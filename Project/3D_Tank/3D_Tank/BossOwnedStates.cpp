...
void Battle::execute(AIController* pBoss, float deltaTime) {
	...
	if (BOSS->aiCount > BOSS->attackTimeDelay()) {
		BOSS->aiCount = 0.0f;
		if (normalshot < 3) {
			pBoss->Attack(BOSS->batteryPosition(), getTargetPos - BOSS->batteryPosition(), 1);  // + 01
			normalshot += 1;
		}
		
		else { 
			pBoss->Attack(BOSS->batteryPosition(), getBOSSHeading,pBoss->getTarget(), 1);  // + 01
			normalshot = 0;
		} 
	}
	////////////////////////Battery follows////////////////////////////
...
void Violent::exit(AIController* pBoss) {
	...
	BOSS->showSuperAttackName(false);
